...
pub trait PathExt {
    ...
    fn join_with_extension<P: AsRef<Path>, S: AsRef<OsStr>>(
        &self,
        name: P,
        extension: S,
    ) -> PathBuf;
...
mod tests {
    ...
    fn paths() {
        ...
        assert_eq!(
            path.join_with_extension("bar", "json"),
            PathBuf::from(format!("foo{SEP}bar.json"))
        );
        assert_eq!(
            path.append(SEP).appended("bar.json"),
            PathBuf::from(format!("foo{SEP}bar.json"))
        );
        assert_eq!(
            path.join("bar").joined("baz"),
            PathBuf::from(format!("foo{SEP}bar{SEP}baz"))
        );
    }
}
