...
impl Gav {
    ...
    pub fn file(&self) -> PathBuf {
        let len = self.group_len.get() as usize
            + 1
            + self.artifact_len.get() as usize
            + 1
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for Visitor {
            ...
            fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                write!(
                    formatter,
                    "a string gav (group:artifact:version[:classifier][@extension])"
                )
            }
            ...
            where
                E: serde::de::Error,
            {
                Gav::_from_str(Cow::Owned(v)).ok_or_else(|| {
                    E::custom(
                        "invalid string gav (group:artifact:version[:classifier][@extension])",
                    )
                })
            }
            ...
            where
                E: serde::de::Error,
            {
                Gav::_from_str(Cow::Borrowed(v)).ok_or_else(|| {
                    E::custom(
                        "invalid string gav (group:artifact:version[:classifier][@extension])",
                    )
                })
...
mod tests {
    ...
    fn file_debug_assert() {
        Gav::new("foo.bar", "baz", "0.1.2-beta", None, None)
            .unwrap()
            .file();
        Gav::new("foo.bar", "baz", "0.1.2-beta", Some("natives"), None)
            .unwrap()
            .file();
        Gav::new("foo.bar", "baz", "0.1.2-beta", None, Some("jar"))
            .unwrap()
            .file();
        Gav::new("foo.bar", "baz", "0.1.2-beta", Some("natives"), Some("jar"))
            .unwrap()
            .file();
    }
    ...
    fn as_str_correct() {
        ...
        assert_eq!(
            Gav::new("foo.bar", "baz", "0.1.2-beta", Some("natives"), None)
                .unwrap()
                .as_str(),
            "foo.bar:baz:0.1.2-beta:natives"
        );
        assert_eq!(
            Gav::new("foo.bar", "baz", "0.1.2-beta", None, Some("jar"))
                .unwrap()
        ...
        assert_eq!(
            Gav::new("foo.bar_ok", "baz", "0.1.2+beta", None, None)
                .unwrap()
                .as_str(),
            "foo.bar_ok:baz:0.1.2+beta"
        );
    }
    ...
    fn modify() {
        let gav = Gav::from_str("foo.bar:baz:0.1.2-beta").unwrap();
        assert_eq!(
            gav.with_group("foo1.bar2").unwrap().as_str(),
            "foo1.bar2:baz:0.1.2-beta"
        );
        assert_eq!(
            gav.with_artifact("baz1").unwrap().as_str(),
            "foo.bar:baz1:0.1.2-beta"
        );
        assert_eq!(
            gav.with_version("0.1.3-alpha").unwrap().as_str(),
            "foo.bar:baz:0.1.3-alpha"
        );
        assert_eq!(
            gav.with_classifier(Some("natives")).unwrap().as_str(),
            "foo.bar:baz:0.1.2-beta:natives"
        );
        assert_eq!(
            gav.with_extension(Some("txt")).unwrap().as_str(),
            "foo.bar:baz:0.1.2-beta@txt"
        );
    }
    ...
    fn canonicalized() {
        assert_eq!(
            Gav::from_str("foo.bar:baz:0.1.2-beta").unwrap(),
            Gav::from_str("foo.bar:baz:0.1.2-beta@jar").unwrap()
        );
    }
}
